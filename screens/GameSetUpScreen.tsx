...
type GameSetUpScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  "Game"
>;

type GameSetUpScreenRouteProp = RouteProp<RootStackParamList, "Game">;

type SlotScreenScreenNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  "Slot"
>;
...
const GameSetUpScreen = () => {
  ...
  const slotNavigation = useNavigation<SlotScreenScreenNavigationProp>();
  ...
  const venue = venues.find(
    (item: Venue) => item.name === route?.params?.item?.area
  );
  ...
  const sendJoinRequest = async (gameId: string) => {
    try {
      const response = await axios.post(
        `http://10.your-ip-address-here:3000/api/games/${gameId}/request`,
        {
          userId,
          comment
        }
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await axios.get(
        `http://10.your-ip-address-here:3000/api/games/${gameId}/requests`
      );
  ...
  const fetchPlayers = async () => {
    try {
      ...
      const response = await axios.get(
        `http://10.your-ip-address-here:3000/api/games/${gameId}/players`
      );
  ...
  useEffect(() => {
    const fetchVenues = async () => {
      try {
        const response = await axios.get(
          "http://10.your-ip-address-here:3000/api/venues"
        );
  ...
  const toggleMatchFullStatus = async (gameId: string) => {
    try {
      const response = await axios.post(
        "http://10.your-ip-address-here:3000/api/games/toggle-match-full",
        { gameId }
      );
    ...
    >
      <ScrollView>
        ...
        >
            ...
            {route?.params?.item?.isUserAdmin && (
              ...
              >
                ...
                <FontAwesome
                  ...
                  name={
                    matchFull || route?.params?.item?.matchFull
                      ? "toggle-on"
                      : "toggle-off"
                  }
                  size={24}
          ...
          >
            <Entypo name="location" size={20} color={theme.text} />
        ...
        >
          ...
          <View>
            ...
            >
              ...
              <Entypo name="chevron-small-right" size={24} color={theme.text} />
            </View>
          </View>
        </View>
        ...
        >
          ...
          >
            ...
            <Ionicons name="earth" size={24} color={theme.text} />
          </View>
          ...
          >
            <View>
              <Image
                ...
                source={{
                  uri: route?.params?.item?.adminUrl
                }}
              />
            </View>
        ...
        {route?.params?.item?.isUserAdmin == true ? (
          <Pressable
            ...
            onPress={() =>
              navigation.navigate("Chat", {
                gameId: route?.params?.item?._id,
                gameName: route?.params?.item?.sport
              })
            }
          >
            <Text style={[styles.gameChatText]}>GAME CHAT</Text>
          </Pressable>
        ) : userRequested ? (
          <Pressable
            style={{
          ...
          >
            ...
            <Pressable
              onPress={() => {
                if (route?.params?.item?.matchFull) {
                  Alert.alert(
                    "Game Full",
              ...
              disabled={route?.params?.item?.matchFull}
              style={{
                backgroundColor: route?.params?.item?.matchFull
                  ? "#ccc"
                  : "#07bc0c",
                marginTop: "auto",
                marginBottom: 30,
            ...
            >
              ...
              >
                {route?.params?.item?.matchFull ? "GAME FULL" : "JOIN GAME"}
              </Text>
            </Pressable>
          </View>
        )}
